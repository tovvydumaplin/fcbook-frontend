...
const createMerchantDiscount = async () => {
  errorMsg.value = "";
  ...
  try {
    ...
    const response = await fetch(`${apiBase}/merchants/discounts`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.message || "Failed to create merchant discount",
      );
    }
    ...
    console.error(err);
    errorMsg.value = "Failed to create merchant discount.";
  } finally {
...
<template>
  ...
  >
    ...
    >
      ...
      <!-- FORM CONTENT  -->
      <form
        @submit.prevent="createMerchantDiscount"
