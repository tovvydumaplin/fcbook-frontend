...
onMounted(() => {
  discountName.value = props.iad.discountName;
  routeId.value = props.iad.routeId;

  if (props.iad.discountType === "percentage") {
    discountType.value = "percentage";
    percentageValue.value = props.iad.discountValue;
    fixedValue.value = "";
  } else {
    discountType.value = "fixed";
    fixedValue.value = props.iad.discountValue;
    percentageValue.value = "";
  }
});
</script>
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <div>
          <h2 class="text-lg font-semibold text-gray-900">Edit IA Discount</h2>
        </div>
        <button
          @click="$emit('close')"
      ...
      <!-- FORM CONTENT  -->
      <form @submit.prevent="editIADiscount" class="flex flex-col p-6 gap-6">
