  ...
  try {
    ...
    if (!response.ok) {
      if (data.errors) {
        ...
        errorMsg.value = data.message || "Failed to create promo.";
      }
      return;
    }
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <div>
          <h2 class="text-lg font-semibold text-gray-900">Create Promo</h2>
        </div>
        <button
          @click="$emit('close')"
      ...
      <form @submit.prevent="createPromo" class="p-6 space-y-6">
        <!-- FORM  -->
        <div>
